...
export const DEMO_PERSONA_TEMPLATES = Object.values(DEMO_PERSONAS)
...
export const FLAVORED_GAME_CONTENT = {
  ...
  'persona-prediction': {
    mild: [
      { text: '갑자기 하루의 자유 시간이 생겼다.', options: ['집에서 푹 쉰다.', '가보고 싶던 장소에 혼자 간다.', '친구에게 연락해 즉석 약속을 잡는다.', '밀린 일을 모두 끝낸다.'] },
      { text: '친구들과 여행 계획을 세우기 시작했다.', options: ['교통편부터 바로 예약한다.', '예산과 일정을 표로 정리한다.', '모두가 원하는 활동부터 묻는다.', '여행 후기와 사진을 천천히 찾아본다.'] },
      { text: '처음 보는 사람이 많은 모임에 도착했다.', options: ['먼저 여러 사람에게 말을 건다.', '아는 사람 옆에서 자연스럽게 합류한다.', '분위기와 사람들을 먼저 관찰한다.', '모두 함께할 수 있는 게임을 제안한다.'] },
      { text: '친구가 갑자기 고민 상담을 요청했다.', options: ['현실적인 해결 방법을 제시한다.', '판단하지 않고 끝까지 들어준다.', '밖으로 데리고 나가 기분을 전환시킨다.', '생각을 정리할 시간을 준 뒤 다시 연락한다.'] },
      { text: '계획에 없던 여행 제안을 받았다.', options: ['날짜만 맞으면 바로 수락한다.', '비용과 일정을 먼저 계산한다.', '같이 가는 사람들의 의견을 확인한다.', '이번에는 거절하고 다음 기회를 잡는다.'] },
    ],
    spicy: [
      { text: '은은한 조명 아래 단둘이 홈 데이트를 하게 됐다.', options: ['함께 요리하며 자연스럽게 시간을 보낸다.', '와인과 영화를 준비해 분위기를 만든다.', '음악을 틀고 오래 대화한다.', '밖으로 나가 늦은 산책을 제안한다.'] },
      { text: '데이트가 끝난 뒤 상대가 “조금 더 같이 있고 싶다”고 말한다.', options: ['나도 그렇다고 솔직하게 표현한다.', '근처에서 조금 더 걸을 장소를 찾는다.', '다음 데이트를 바로 약속하고 집에 간다.', '다음 날 일정을 확인한 뒤 결정한다.'] },
      { text: '연인과 다툰 뒤 먼저 화해해야 한다.', options: ['솔직한 대화부터 시작한다.', '포옹해도 되는지 먼저 물어본다.', '손편지나 작은 선물로 마음을 표현한다.', '서로 진정할 시간을 가진 뒤 연락한다.'] },
      { text: '호감 있는 사람의 귓가에 한마디를 속삭일 수 있다.', options: ['“오늘 계속 보고 싶었어.”', '“너랑 있으면 정말 편해.”', '“조금 더 가까이 있어도 돼?”', '말하지 않고 웃으며 바라본다.'] },
      { text: '상대가 갑자기 손깍지를 끼었다.', options: ['자연스럽게 손을 마주 잡는다.', '장난으로 놀리며 분위기를 푼다.', '부끄러워하면서도 그대로 있는다.', '괜찮은지 서로의 반응을 확인한다.'] },
      { text: '친구들이 최근 보낸 가장 설레는 메시지를 보여 달라고 한다.', options: ['당당하게 보여준다.', '일부만 가리고 보여준다.', '내용만 말로 설명한다.', '끝까지 비밀로 한다.'] },
    ],
  },
  liar: {
    mild: [
...
export const DEMO_GAME_CATALOG = [
  { id: 'persona-impostor', path: '/games/demo/persona-impostor', group: 'Persona Games', emoji: '🕵️', title: '너 누구야?', desc: 'Persona를 훔친 Impostor 찾기' },
  { id: 'persona-prediction', path: '/games/demo/persona-prediction', group: 'Persona Games', emoji: '🔮', title: '너라면?', desc: 'Persona를 보고 실제 선택 예측하기' },
