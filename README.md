...
현재 코드는 **개정 전 기획안** 기준이다. 아래가 맞춰야 할 차이다.

```
□ 인원 1~6명            지금은 정확히 5명이 아니면 어느 단계도 넘어가지 않고
                        리포트가 400을 낸다. MAX_PLAYERS 상수를 게이트 조건에서
                        빼고 len(players)로 계산해야 한다

□ localStorage 복구      새로고침하면 playerId가 사라져 그 사람은 복귀가 불가능하다

□ 집계만 공개            지금은 누가 뭘 골랐는지 이름이 다 보인다. 개인 선택이
                        공개되면 사람들은 보이고 싶은 모습을 고르게 되어
                        능력치가 오염된다. 3 : 2 형태로만 보여준다   §3-4
...
□ 문항·소재 생성         호스트가 적은 프로젝트로 LLM이 문항과 게임 소재를 만든다   §5
□ 리포트 생성            사전 문장 대신 그 판에서 있었던 일로 쓴다   §5-2

□ 연출                   제출 표시 · 카운트다운 · 집계 차오름 · 색 전환   §13
```

**앞의 세 개가 먼저다.** 인원과 재접속이 막혀 있으면 나머지를 아무리 만들어도
끝까지 돌려볼 수가 없다.
