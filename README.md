...
처음 만난 사람들이 **모여 앉아** 18분짜리 아이스브레이킹을 함께 하면, 각자의
캐릭터 카드와 팀 리포트가 나오는 웹 서비스.

**진행은 서비스가 맡는다.** 아이스브레이킹은 보통 사회자가 필요하고, 그 사람은
진행하느라 정작 어울리지 못한다. 여기서는 규칙 안내도 순서도 시작 신호도 앱이
낸다 — 사회자 없이 판이 돌아간다.

**말은 사람끼리 오간다.** 라이어 게임은 서로 제시어를 떠보며 추리하는 게임이고,
눈치 게임도 서로를 보면서 한다. 앱이 안 쓰는 것은 **음성 인식**이다: 오간 말을
받아 적는 대신 **선택과 추측만으로** 능력치를 만든다.
